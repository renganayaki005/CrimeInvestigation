<%-- 
    ...
    Created on : Apr 5, 2020, 5:08:35 PM
    Author     : RengaNayagi
--%>
...
table, thead, tbody, tr { width: 100%; }
  ...
  <div class="signout"><a href="signOut.jsp">Sign out</a></div>
        ...
        %>
<form method="post"  name="viewEvidence">
  <div class="container1">
   ...
   Class.forName("oracle.jdbc.driver.OracleDriver");     
   Connection con=(Connection)DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:XE","system","renga");
   PreparedStatement pstmt=con.prepareCall("select * from evidence where caseId='"+caseId+"'");
   ResultSet rs=pstmt.executeQuery();
   if(!rs.isBeforeFirst())
   {
       %><center><h3>No Evidence added</h3></center><%
   }
   else{
       %>
       <table id="table" border="1">
            ...
            <%
             while(rs.next())
             {
                 %>
                 <tr>
                     <td><%out.print(rs.getString("evidenceId"));%></td>
                     <td><%out.print(rs.getString("evidenceType"));%></td>
                     <td><%out.print(rs.getString("evidenceDesc"));%></td>
                      <%
                      Blob image=null;
                      byte[ ] imageData = null ;
                      ...
                      if(imageData!=null){
                     %>
                     <td><a href="viewEvidenceImage.jsp?evidenceId=<%out.print(rs.getString("evidenceId"));%>"><center><img src="view-image.png" width="70" height="50"></center></a></td>
                     <%
                      }
                      else{
                      %>
